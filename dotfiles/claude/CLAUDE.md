...
- **LaTeX compilation**: Use the `pdfcompile file.tex` zsh function (defined in `~/.zshrc`) instead of calling `pdflatex` directly. It runs `pdflatex` twice (for cross-references) and removes aux files (`.aux .log .out .toc .fls .fdb_latexmk .synctex.gz .bbl .blg` etc.), leaving only the `.tex` source and the `.pdf` output.
