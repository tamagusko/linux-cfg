...
description: Use when checking whether lecture decks have gone stale between semesters — "are the slides still current", "what needs updating for 2027", "check the decks for outdated claims", "did any model names or prices change". Sweeps Quarto decks for time-sensitive claims and reports candidates for human re-verification.
---
...
```bash
python3 ~/.claude/skills/lecture-currency/scripts/currency_sweep.py [COURSE_ROOT]
```
