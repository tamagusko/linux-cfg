---
name: lecture-gate
description: Run the acceptance gate on a Quarto RevealJS lecture deck for the IME AI-Applied-to-Transportation course, checking it against lecture_pattern.md before it can move from built to gated. Use when a deck is ready for review — "gate class 7", "check this deck against the pattern", "is class 9 ready", "run the cold gate", "review my lecture before I teach it" — and when auditing a deck that has drifted. The gate is deliberately a separate pass from the build: a model that just wrote a deck is the worst judge of whether it is right. Not for authoring a deck from scratch, and not for grading student work.
---
