...
description: Use when the user asks to "write the abstract", "draft an abstract", "check my abstract", "improve this abstract", "is my abstract too long", or needs an abstract for a paper, thesis chapter, or conference submission in transportation, civil engineering, AI/ML or urban analytics. Two modes, draft from a manuscript and review of an existing abstract, both ending in an abstract plus a counted audit table. Not for the final polish of a settled manuscript (paper-self-review runs after this), not for someone else's abstract (paper-review), not for choosing the venue (journal-selection).
version: 0.1.0
---
...
```
python3 ~/.claude/skills/abstract-writing/scripts/abstract_audit.py TAGGED.txt --tex main.tex --limit N
```
