---
name: paper-review
description: Write a peer review of someone else's manuscript, in Tiago's reviewing voice, for journals and conferences in transportation, civil engineering, AI/ML and urban analytics. Use this whenever the user is refereeing a paper: "review this paper", "write reviewer comments", "referee this manuscript", "I have a review due", "faz o parecer", a submission ID plus a PDF, or a manuscript handed over with no instruction beyond wanting an opinion on it. Also use for revision rounds, where the job is checking a rebuttal against the previous review. Governing rule: catch every error, and cut every comment the paper would be fine without. Not for reviewing the user's OWN draft (that is paper-self-review), not for responding to reviewers (that is review-response), and not for code review.
---
